...
slack_client = SlackClient('xoxb-312941077155-363161276545-DUvCTfrdgEWkvJdtyHilCbQM')
# starterbot's user ID in Slack: value is assigned after the bot starts up
starterbot_id = None

# constants
RTM_READ_DELAY = 1  # 1 second delay between reading from RTM
ASK_QUESTION_COMMAND = ["test", "qa", "ask"]
EXAMPLE_COMMAND = "do"
MENTION_REGEX = "^<@(|[WU].+?)>(.*)"
...
def load_question_and_answers():
    ...
    path = get_path()
    question_and_answer_path = path + 'qa.dat'
    question_and_answer_path = path + 'questions.json'
    print("loading pre-exacted question and answers")
    with open(question_and_answer_path, "rb") as f:
        mapped_question_and_answer = pickle.load(f)
    return mapped_question_and_answer
...
def handle_command(command, channel):
    ...
    if command in ASK_QUESTION_COMMAND:
        mapped_question_and_answer = load_question_and_answers()
